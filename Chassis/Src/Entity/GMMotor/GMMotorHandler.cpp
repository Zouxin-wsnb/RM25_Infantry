...
void GMMotorHandler::sendControlData()
{
    ...
    // can_sendData(&hcan1, 0x200, can1_send_data_0, 8); // 向CAN1发送数据，电机控制报文0x200
    // can_sendData(&hcan1, 0x1FF, can1_send_data_1, 8); // 向CAN1发送数据，电机控制报文0x1FF

    // can_sendData(&hcan2, 0x200, can2_send_data_0, 8); // 向CAN2发送数据，电机控制报文0x200
    // can_sendData(&hcan2, 0x2FF, can2_send_data_1, 8); // 向CAN2发送数据，电机控制报文0x2FF
}
...
void GMMotorHandler::updateFeedback()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (GMMotorList[0][i] != nullptr)
        {
            GMMotorList[0][i]->motorFeedback.last_ecd = GMMotorList[0][i]->motorFeedback.ecd;
            GMMotorList[0][i]->motorFeedback.ecd = can1_receive_data[i].ecd;
            GMMotorList[0][i]->motorFeedback.speed_rpm = can1_receive_data[i].speed_rpm;

            GMMotorList[0][i]->motorFeedback.lastPositionFdb = GMMotorList[0][i]->motorFeedback.positionFdb;
            GMMotorList[0][i]->motorFeedback.lastSpeedFdb = GMMotorList[0][i]->motorFeedback.speedFdb;
            // 位置反馈，转换为弧度，并限幅至-PI到PI
            GMMotorList[0][i]->motorFeedback.positionFdb = (float)(can1_receive_data[i].ecd) * RawPosToRad; // 对于6020，这里是绝对位置，范围[0, 2*PI]
            // GMMotorList[0][i]->motorFeedback.positionFdb = Math::LoopFloatConstrain((float)(can1_receive_data[i].ecd) * RawPosToRad, -Math::Pi, Math::Pi); // 对于6020，这里是绝对位置
            GMMotorList[0][i]->motorFeedback.speedFdb = can1_receive_data[i].speed_rpm * RawRpmToRadps;
            GMMotorList[0][i]->motorFeedback.currentFdb = can1_receive_data[i].given_current;

            GMMotorList[0][i]->motorFeedback.temperatureFdb = (float)can1_receive_data[i].temperate;
        }
        // 处理CAN2的电机
        if (GMMotorList[1][i] != nullptr)
        {
            GMMotorList[1][i]->motorFeedback.last_ecd = GMMotorList[1][i]->motorFeedback.ecd;
            GMMotorList[1][i]->motorFeedback.ecd = can1_receive_data[i].ecd;
            GMMotorList[1][i]->motorFeedback.speed_rpm = can1_receive_data[i].speed_rpm;

            GMMotorList[1][i]->motorFeedback.lastPositionFdb = GMMotorList[1][i]->motorFeedback.positionFdb;
            GMMotorList[1][i]->motorFeedback.lastSpeedFdb = GMMotorList[1][i]->motorFeedback.speedFdb;
            // 位置反馈，转换为弧度，并限幅至-PI到PI
            GMMotorList[1][i]->motorFeedback.positionFdb = (float)(can2_receive_data[i].ecd) * RawPosToRad; // 对于6020，这里是绝对位置，范围[0, 2*PI]
            // GMMotorList[1][i]->motorFeedback.positionFdb = Math::LoopFloatConstrain((float)(can2_receive_data[i].ecd) * RawPosToRad, -Math::Pi, Math::Pi); // 对于6020，这里是绝对位置
            GMMotorList[1][i]->motorFeedback.speedFdb = can2_receive_data[i].speed_rpm * RawRpmToRadps;
            GMMotorList[1][i]->motorFeedback.currentFdb = can2_receive_data[i].given_current;

            GMMotorList[1][i]->motorFeedback.temperatureFdb = (float)can2_receive_data[i].temperate;
        }
    }
}

...
Monitor::Monitor_Status Monitor::Log_Messages(LOGLEVEL level, uint8_t *message)
{
    ...
    if (size >= MONITOR_LOG_MAX_SIZE)
    {
        return Monitor_LOG_ERROR; // 如果消息太长，返回错误
    }
    uint8_t buffer[MONITOR_LOG_MAX_SIZE] = {0};
...
void Monitor::Monitor_Run()
{
    if (Monitor_Counter == 1 && Is_Robot_Init == true) // 控制监测频率
    {
        ...
        // dr16遥控器数据监测
        if (fabs(Monitor_Time - RemoteController_Probe.Update_Time_s) > 0.1f)
        {
            if (RemoteController_Probe.Status == Probe_NORMAL || RemoteController_Probe.Is_Loged == false)
            {
                log_status = Log_Messages(ERROR, (uint8_t *)("RemoteController Data is not updated!\r\n"));
                RemoteController_Probe.Status = Probe_ERROR;

                if (log_status == Monitor_LOG_OK)
                {
                    RemoteController_Probe.Is_Loged = true;
                }
            }
        }
        else
        {
            if (RemoteController_Probe.Is_Loged == true && RemoteController_Probe.Status == Probe_ERROR)
            {
                RemoteController_Probe.Is_Loged = false;
                RemoteController_Probe.Status = Probe_NORMAL;

                Log_Messages(INFO, (uint8_t *)("RemoteController Data update restored!\r\n"));
            }
        }
        // 电机堵转监测
